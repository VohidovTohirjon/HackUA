...
export default function MetricsPanel() {
  ...
  const [source, setSource] = useState("local review");

  useEffect(() => {
    fetchMetrics(evaluationMetrics).then((result) => {
  ...
  return (
    <section id="metrics" className="border-y border-slate-200 bg-white py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        />
        <div className="mb-5 rounded-xl border border-cyan-200 bg-cyan-50 p-4 text-sm font-black text-care-cyan">
          Metrics source: {source === "api" ? "CareBridge service" : "local demo set"}
        </div>
